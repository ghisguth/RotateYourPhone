...
# Combine options: ./process_video.sh "input_video.mp4" --skip-thumbnails --quality best --keep-intermediate
#
...
INPUT_FILE=""
QUALITY_LEVEL="medium" # Default quality: fast, medium, high, best
  ...
  case "$1" in
    ...
    *) # Positional argument (input file)
      if [ -z "$INPUT_FILE" ]; then
        ...
        echo "Usage: $0 <input_video_file> [--skip-thumbnails] [--skip-rotation] [--quality <level>] [--keep-intermediate] [--disable-hwaccel]"
        exit 1
      fi
      ;;
  esac
...
if $USE_VIDEOTOOLBOX; then
  ...
  case "$QUALITY_LEVEL" in
    fast)   FINAL_CRF=24; FINAL_PRESET="fast";;
    medium) FINAL_CRF=22; FINAL_PRESET="medium";;
    high)   FINAL_CRF=20; FINAL_PRESET="slow";;
    best)   FINAL_CRF=18; FINAL_PRESET="slower";;
    *) echo "Error: Invalid quality level '$QUALITY_LEVEL'." >&2; exit 1;;
  esac
...
FINAL_HEVC_OUTPUT_FILE="${INPUT_BASE}-RotateYourPhone-${QUALITY_LEVEL}.mp4"
...
# Add specific thumbnail crop
THUMBNAILS_VF_FILTER+="${THUMBNAILS_VF_FILTER:+,}crop=608:1080"
...
if ! $SKIP_ROTATION; then
  ...
  FFMPEG_PRORES_CMD+=(-c:v "$PRORES_ENCODER" "${PRORES_OPTS[@]}" -c:a pcm_s16le -ar 48000 -y "$INTERMEDIATE_PRORES_FILE")
...
# --- Step 2: Concatenate and Encode to HEVC 10-bit ---
...
CONCAT_FILTER_COMPLEX="[0:v]scale=${TARGET_WIDTH}:${TARGET_HEIGHT}:force_original_aspect_ratio=decrease,pad=${TARGET_WIDTH}:${TARGET_HEIGHT}:(ow-iw)/2:(oh-ih)/2[v0];[v0][0:a][1:v][1:a]concat=n=2:v=1:a=1[v][a]"
